...
traker has been written using node.js with express, ejs templating language and a number of libraries from npm.
The authentication in the app is done by using passport library and local strategy in conjunction with express-session library and Firestore-store npm library to 
store user sessions at Firestore.


The app handles the process of adding text data to Firestore by using Firestore API. For image uploads multer-firebase-storage library was used.
...
inside the app or rewrite the code and make the app communicate with Google Cloud storage directly (then admin SDK will work). That is because tracker uses client SDK for Firebase storage, which demands a user authentication or using an admin SDK and Google Cloud storage bucket directly if you want to restrict access to your storage bucket in security rules but make the app capable of bypassing the security rules. If you define your security rules for Firebase Storage as follows, then the app will work just fine without modifications, but be aware that this allows anyone to read and change data in your storage bucket. 
...
no matter what you've set. For example, if you set security rule like below one for Firestore, the app will work fine and nobody will have access to your Firestore instance except the app.
...
admin: true  //if set to false, user will not see the whole content, such as "Employees" page, therefore will not be able to create and delete users and also delete projects. Set this option to true only for administrator accounts or for the first user you are creating manually in Firestore.
```
3. Create a .env file which will consist the folowing environment variables:
...
ALG=      //algorithm used for generating the hash (for example 'sha' algorithms)
```
